...
- Desktop reset: **Reset demo** reloads the bundled labels. **Start for real** discards the in-memory sample and returns to window selection.
- Storage: the website demo has no storage. The desktop demo uses only `demo:lens:speech-rate`; it never reads or writes `lens:speech-rate` while the demo banner is displayed.
